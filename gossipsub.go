...
// When this is enabled, the router will try to batch messages in a single message so that
// it has less packet header overhead.
func WithMessageBatching(messageBatching bool) Option {
	return func(ps *PubSub) error {
		gs, ok := ps.rt.(*GossipSubRouter)
		if !ok {
			return fmt.Errorf("pubsub router is not gossipsub")
		...
		gs.messageBatching = messageBatching
